...
**✅ RESOLVED 2026-08-15 (later the same day) — the final-review fix landed as `2afde94`, is pushed, deployed and verified live. Nothing outstanding from it.** For the record, since the previous version of this note asked the next session to go chase it: the `fix-final-review` subagent had in fact **written every fix to the working tree but never committed them**, so `git log` showed nothing while 12 modified files sat uncommitted. If a handed-off agent's commit appears missing in future, check `git status` before concluding the work wasn't done — an uncommitted working tree looks identical to "the agent never ran". The fixes were verified (build clean, 51/51 Playwright, and each of the six findings checked against the live domain in a browser) and committed as `2afde94`.

  **Verified live on truthcaregroup.co.uk, not just locally:** `/a-day-at-beaconsfield` now has a real `h1 → h2 ×4` outline; the footer carries the `/accessibility` link (`/accessibility` returns 200); both header toggles' aria-labels lead with their visible text ("Aa — switch to large text size", "Motion on — turn off animation"); `/reviews` renders the permission note from content rather than a hardcoded fallback; and — the one the reviewer flagged as plausible-but-unverified — **at 1201px, toggling large text no longer reopens the header overflow bug**: no horizontal overflow and the Brochure button stays fully on screen, because the header row now wraps instead of clipping.
...
## Tickets service — AI call answering + email ticketing (built 2026-09-05, deployed 2026-09-06)

**Status: LIVE and proven end-to-end.** `feat/tickets` merged to `main` and deployed. Board: https://tickets.truthcaregroup.co.uk (Sign in with Microsoft — staff-allowlist only). Phone: `+44 151 453 5412` ("Truth Care Group – Weston-super-Mare" in Retell, a Liverpool number — see the Twilio note below for why). The user rang it 2026-09-06, logged a general-enquiry ticket (TC-1) and received the staff notification email — the full call → ticket → email pipeline is confirmed working with a real call, not just tests. Spec: `docs/superpowers/specs/2026-09-05-ai-call-ticketing-design.md`. Plan: `docs/superpowers/plans/2026-09-05-ai-call-ticketing.md`. Full build history and every review finding: `.superpowers/sdd/progress.md`.

**Hosting.** Vercel project **`truthcare-tickets`** (root dir `tickets`), deploying from `main`. Neon Postgres (`truthcare-tickets-db`, London/`lhr1`, free tier) provisioned via Vercel's Storage integration — `DATABASE_URL` is auto-managed there, not a manually-set env var. Domain `tickets.truthcaregroup.co.uk` → CNAME `tickets` → `204923897a98b937.vercel-dns-016.com.` (added at GoDaddy, existing SPF/MX/M365 records untouched). Crons in `tickets/vercel.json`: `/api/cron?job=email` and `?job=notifications` every 5 min, `?job=retention` weekly Sunday 03:00, `?job=digest` daily at 07:00 UTC — these fire automatically on Vercel, nothing further to do.

**Daily digest email — added 2026-09-06.** `?job=digest` sends one email a day (07:00 UTC) summarising tickets created/closed in the last 24h plus every currently-open ticket. To `kumi@truthcaregroup.co.uk` and `joanne@truthcaregroup.co.uk`, bcc `infotech@truthcaregroup.co.uk` — overridable via `DIGEST_TO`/`DIGEST_BCC` env vars (comma-separated) if the list changes. Sent directly via `lib/digest.js`, not queued through `pending_notifications` like every other email — it isn't tied to one ticket's lifecycle. `lib/graph.js`'s `sendMail` gained `bcc` support for this.

**Retell.** Agent id **`agent_4d82b100b4d5daca406a5f317b`**, currently on **published V2 ("Number fix + FAQ facts")** — the phone number's Inbound Call Agent is manually pointed at V2, see the version-binding gotcha below. Model: Claude 5 Sonnet. Voice: **Amy (UK)**, an ElevenLabs-native British voice (`11labs-Amy`) — chosen after a live call reported Willa (Retell's own "British" stock voice) sounding Australian; Willa/Maren are Retell's only two natively-British stock voices and both sounded off, so we switched provider rather than picking between the same two. `create_ticket` and `lookup_ticket` custom functions are wired to `https://tickets.truthcaregroup.co.uk/api/phone?action=create_ticket` / `?action=lookup_ticket` with parameter schemas matching `lib/phone.js` exactly (they started as an unconfigured placeholder named `log_ticket` pointing at `example.com` — that was replaced, not extended). Webhook URL: `https://tickets.truthcaregroup.co.uk/api/phone?action=webhook`, `call_analyzed` event enabled. Recording is off, transcript retention 7 days.

**Callback-number fix — 2026-09-06, live example of the bug caught same day.** The caller (TC-2) said `07841636133`; the ticket recorded `...636113` — Retell's speech-to-text mis-heard one digit while transcribing a spoken number. Fixed in the prompt (`docs/agent-prompt.md`, pasted into the live agent as V2): the agent now reads the caller's number back from Retell's own `{{user_number}}` system variable (the real caller ID for the call) and asks for a yes/no, rather than asking the caller to speak digits that then get transcribed. `caller_phone` is only passed to `create_ticket` when the caller explicitly wants a different number, in which case it's read back digit-by-digit before moving on. No code change was needed — `lib/phone.js`'s existing `callerPhoneFrom()` (a spoken number wins over caller ID, spec §4.3) was already correct for the genuine-override case; the fix is that the agent now avoids transcribing a number at all in the common case. Residual, not fixed: the same call badly mis-heard a **name** ("Kumi Pillay" → "Koo Mee Peelay") — there's no caller-ID equivalent for a spoken name, so this needs a different fix later (e.g. asking the agent to confirm the spelling of unusual names) if it keeps happening.

**FAQ facts — added 2026-09-06.** A "# Facts you can share" section in the prompt (address, contact emails, conditions supported, free initial assessment, CQC registration — all sourced from the site's own committed content, not invented) lets the agent answer a simple factual question without logging it as a ticket. Explicitly scoped: never used to confirm/deny anything about a named person, and any question beyond these facts still gets logged normally rather than guessed at.

**Gotcha — publishing does NOT move the phone number.** Retell's Publish button creates a new published version (V0 → V1 → …) but leaves the phone number's "Inbound Call Agent" pinned to whichever version it already pointed at. After every future Publish, go to **Phone Numbers → Truth Care Group - Weston-super-Mare → Inbound Call Agent** and manually reselect the new version (e.g. `.../V2`) — otherwise real calls keep running the old version indefinitely with no error or warning. Confirmed twice this session: the phone stayed on V0 through two publishes until the dropdown was changed by hand.

**Entra app registration (Graph, client-credentials).** Name: "Truth Care Tickets Service". Tenant `truthcaregroup.co.uk`, tenant ID `25f9c194-c7b6-4653-b98b-eb88fb7f1a47`, client ID `c017fdfe-38b4-44da-baa3-ae24e99bb765`. Application permissions `Mail.Read` + `Mail.Send`, admin-consented. Client secret named "tickets-service prod", expires 2028-09-05 — put a reminder in before then, nothing alerts on expiry and mail will silently stop flowing. Redirect URI `https://tickets.truthcaregroup.co.uk/api/auth?action=callback`.

**Vercel environment variables — all set (Production + Preview):** `MICROSOFT_TENANT_ID`, `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET`, `MAILBOX_ADDRESS=infotech@truthcaregroup.co.uk`, `TICKETS_ADDRESS=tickets@truthcaregroup.co.uk`, `RETELL_API_KEY` (key named `tickets-service` in Retell's API Keys page), `RETELL_WEBHOOK_SECRET` (the pre-existing "Secret Key" Retell had already tagged Webhook), `JWT_SECRET`, `CRON_SECRET`, `APP_URL=https://tickets.truthcaregroup.co.uk`. `DATABASE_URL` comes from the Neon integration, not set by hand. `ANTHROPIC_API_KEY` was **not** set — email classification runs on the regex fallback; add it later if the AI classifier is wanted.

**Staff seeded — three admins, all confirmed by the user or the site's own content.** `kumi@truthcaregroup.co.uk` (Dr Kumi Pillay, Founder — from `support.ts`), `infotech@truthcaregroup.co.uk` (Paul McWilliam), `joanne@truthcaregroup.co.uk` (Joanne Bray, Registered Manager — email confirmed directly by the user 2026-09-06, aliases `jo`/`joanne`). All three receive new-ticket emails and can sign into the board and issue email commands. Anyone else who should triage tickets still needs adding — do it with `cd tickets && vercel env pull .env.production.local --environment=production --yes && set -a && source .env.production.local && set +a && node scripts/seed-staff.js "<Name>" <email> <admin|agent> <aliases>` then delete the pulled `.env.production.local` file (it's git-ignored but holds live secrets).

**Exchange Online config — done 2026-09-06.** Both commands run by Paul McWilliam himself (needed his own MFA, could not be scripted unattended): `Set-OrganizationConfig -SendFromAliasEnabled $true` succeeded, and `New-ApplicationAccessPolicy -AppId c017fdfe-38b4-44da-baa3-ae24e99bb765 -PolicyScopeGroupId infotech@truthcaregroup.co.uk -AccessRight RestrictAccess` created and confirmed (`IsValid: True`) — the app can now only touch `infotech@truthcaregroup.co.uk`, and sending as the `tickets@` alias is allowed.

**Still outstanding — cannot be done from an unattended session:**
- [ ] Telephony: landline forward-on-no-answer (~20 s) and out-of-hours forward → `+44 151 453 5412`.
- [ ] Manual acceptance (spec §9) — partially done: one call (TC-1, general enquiry) confirmed end-to-end 2026-09-06. Still to try: a referral call and a staff-sickness call (different categories/priority paths), an `assign`/`close` reply by email, and a caller-side closure email.
- [ ] Decide whether to add `ANTHROPIC_API_KEY` for AI email classification (optional — regex fallback works without it).
...
**Testing.** `cd tickets && npm test` — 119 tests, 118 pass, 1 skipped unless `DATABASE_URL` points at a **scratch** Neon branch (the integration test truncates every table — never run this against the real `truthcare-tickets-db`). Phone door without a call: `node scripts/simulate-call.js --url https://tickets.truthcaregroup.co.uk/api/phone --key $RETELL_API_KEY` (or `--dry` to print the signed requests).
