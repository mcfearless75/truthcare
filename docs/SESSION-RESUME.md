...
**Status: LIVE.** `feat/tickets` merged to `main` and deployed. Board: https://tickets.truthcaregroup.co.uk (Sign in with Microsoft — staff-allowlist only). Phone: `+44 151 453 5412` ("Truth Care Group – Weston-super-Mare" in Retell, a Liverpool number — see the Twilio note below for why). Spec: `docs/superpowers/specs/2026-09-05-ai-call-ticketing-design.md`. Plan: `docs/superpowers/plans/2026-09-05-ai-call-ticketing.md`. Full build history and every review finding: `.superpowers/sdd/progress.md`.

**Hosting.** Vercel project **`truthcare-tickets`** (root dir `tickets`), deploying from `main`. Neon Postgres (`truthcare-tickets-db`, London/`lhr1`, free tier) provisioned via Vercel's Storage integration — `DATABASE_URL` is auto-managed there, not a manually-set env var. Domain `tickets.truthcaregroup.co.uk` → CNAME `tickets` → `204923897a98b937.vercel-dns-016.com.` (added at GoDaddy, existing SPF/MX/M365 records untouched). Crons in `tickets/vercel.json`: `/api/cron?job=email` and `?job=notifications` every 5 min, `?job=retention` weekly Sunday 03:00 — these fire automatically on Vercel, nothing further to do.
...
**Staff seeded so far — incomplete, needs the user.** Only two rows exist in the `staff` table, both admin: `kumi@truthcaregroup.co.uk` (Dr Kumi Pillay, Founder — confirmed real from the site's own `support.ts` content) and `infotech@truthcaregroup.co.uk` (Paul McWilliam). **Joanne Bray (Registered Manager) and anyone else who should triage tickets are not seeded** — their email addresses weren't confirmed anywhere in the repo, so they were deliberately not guessed. Add them with `cd tickets && vercel env pull .env.production.local --environment=production --yes && set -a && source .env.production.local && set +a && node scripts/seed-staff.js "<Name>" <email> <admin|agent> <aliases>` then delete the pulled `.env.production.local` file (it's git-ignored but holds live secrets).
...
- [ ] Seed the remaining staff (Joanne Bray et al.) once their real email addresses are confirmed.
- [ ] Telephony: landline forward-on-no-answer (~20 s) and out-of-hours forward → `+44 151 453 5412`.
- [ ] Manual acceptance (spec §9): ring from a mobile, let it overflow, complete one referral call and one staff-sickness call; confirm both emails arrive at the seeded staff, `assign` and `close` replies work, the ticket shows on the board, the caller receives the closure email.
