...
**Retell.** Agent id **`agent_4d82b100b4d5daca406a5f317b`**, currently on **published V1** (the phone number's Inbound Call Agent is manually pointed at V1 — see the version-binding gotcha below). Model: Claude 5 Sonnet. Voice: **Amy (UK)**, an ElevenLabs-native British voice (`11labs-Amy`) — chosen after a live call reported Willa (Retell's own "British" stock voice) sounding Australian; Willa/Maren are Retell's only two natively-British stock voices and both sounded off, so we switched provider rather than picking between the same two. `create_ticket` and `lookup_ticket` custom functions are wired to `https://tickets.truthcaregroup.co.uk/api/phone?action=create_ticket` / `?action=lookup_ticket` with parameter schemas matching `lib/phone.js` exactly (they started as an unconfigured placeholder named `log_ticket` pointing at `example.com` — that was replaced, not extended). Webhook URL: `https://tickets.truthcaregroup.co.uk/api/phone?action=webhook`, `call_analyzed` event enabled. Recording is off, transcript retention 7 days.
