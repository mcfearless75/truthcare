...
12. **NEW 2026-08-13 — the Search Console property is on `www`, but the whole site canonicalises to the apex. Fix by adding a Domain property.**
    ...
    **Two fixes, both worth doing:**
    1. **Add a Domain property** (`sc-domain:truthcaregroup.co.uk`) via a DNS TXT record at the registrar. It covers apex + www + http + https in one property and is immune to this class of problem. Keep the old www property for historical continuity.
    2. **Make www 301-redirect to the apex** in Vercel (set the apex as primary). The canonical tags already tell Google which to prefer, so this is belt-and-braces rather than urgent — but serving byte-identical content on two hostnames with no redirect is a duplicate-content pattern worth closing.
