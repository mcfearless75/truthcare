...
12. **NEW 2026-08-13 — THERE IS NO SEARCH CONSOLE PROPERTY. This blocks the whole indexing workstream.** Checked all three Google accounts signed into the client's Chrome — `paulmc18@gmail.com`, `infotech@copelandgroupservices.co.uk`, `paul@evotekinnovations.com`. **Every one shows the "Add a website" welcome screen, i.e. zero properties.** `sc-domain:truthcaregroup.co.uk` returns "you don't have access to this property".

    **This corrects an earlier note in this same doc**, which said Search Console was "discovered already connected" with 90 days of data (134 clicks, 1.08k impressions). That data was real enough to ground two content changes, but it did **not** come from any Search Console property on these accounts — most likely from the Wix SEO dashboard while the site was still on Wix. Treat the earlier GSC claim as unverified.

    Consequences, in order:
    - Step 8 of the cutover checklist ("submit the sitemap to GSC, request re-indexing") is not merely undone, it is **currently impossible**. Do not advise anyone to "request re-indexing" until a property exists.
    - Nobody can see impressions, queries, coverage errors or manual actions for the live site.
    - **Google is still serving the pre-cutover Wix page title** — a live search returns the homepage as *"…Residential Rehabilitation **Bristol**"* with a Bristol-led description, which is Wix's old metadata, not this build's. Nothing has prompted a recrawl.

    **Fix:** create a property and verify it. Verification is easy here — either a DNS TXT record at the registrar (best; covers every subdomain and both http/https), or a meta tag in `app/layout.tsx`, which is fully within this repo's control and ships on the next deploy. **The blocking question is whose Google account should own it**, since it is a client business asset. Ask, then do it, then submit `https://truthcaregroup.co.uk/sitemap.xml`.
