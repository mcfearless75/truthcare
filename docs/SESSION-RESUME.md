...
- [ ] Exchange Online PowerShell, run by a real admin with MFA: `Set-OrganizationConfig -SendFromAliasEnabled $true` (without it, sending as `tickets@` is rejected `SendAsDenied`) and `New-ApplicationAccessPolicy -AppId c017fdfe-38b4-44da-baa3-ae24e99bb765 -PolicyScopeGroupId infotech@truthcaregroup.co.uk -AccessRight RestrictAccess` (scopes the app to `infotech@` only, no tenant-wide mailbox access).
